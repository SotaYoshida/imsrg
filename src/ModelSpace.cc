...
ModelSpace::ModelSpace(const ModelSpace& ms)
  :
    ...
    Orbits(ms.Orbits), Kets(ms.Kets), Kets3(ms.Kets3), KetIndexLookup(ms.KetIndexLookup),
...
ModelSpace::ModelSpace(ModelSpace&& ms)
  :
    ...
    dE3max(ms.dE3max), occnat3cut(ms.occnat3cut), e_fermi(ms.e_fermi), norbits(ms.norbits), hbar_omega(ms.hbar_omega),
    target_mass(ms.target_mass), target_Z(ms.target_Z), Aref(ms.Aref), Zref(ms.Zref),
    Orbits(std::move(ms.Orbits)), Kets(std::move(ms.Kets)), KetIndexLookup(std::move(ms.KetIndexLookup)),
...
ModelSpace::ModelSpace(int emax, int e2max, int e3max, std::vector<std::string> hole_list, std::vector<std::string> valence_list)
  :  Emax(emax), E2max(e2max), E3max(e3max), Lmax(emax), Lmax2(emax), Lmax3(emax), OneBodyJmax(0), TwoBodyJmax(0), ThreeBodyJmax(0), EmaxUnocc(emax), dE3max(999), occnat3cut(-1.0), norbits(0), hbar_omega(20), target_mass(16),
...
{
  //   SetUpOrbits();
  Init(emax, e2max, e3max, hole_list, hole_list, valence_list); // Init version 3  (int, vector<string>, vector<string>, vector<string> )
}
...
{
  //   SetUpOrbits();
  Init(emax, e2max, e3max, hole_list, core_list, valence_list); // Init version 3  (int, vector<string>, vector<string>, vector<string> )
}

// Most conventient interface
ModelSpace::ModelSpace(int emax, int e2max, int e3max, std::string reference, std::string valence)
  : Emax(emax), E2max(e2max), E3max(e3max), Lmax(emax), Lmax2(emax), Lmax3(emax), OneBodyJmax(0), TwoBodyJmax(0), ThreeBodyJmax(0), EmaxUnocc(emax), dE3max(999),  occnat3cut(-1.0), hbar_omega(20),
  sixj_has_been_precalculated(false),moshinsky_has_been_precalculated(false), scalar_transform_first_pass(true), scalar3b_transform_first_pass(true),tensor_transform_first_pass(40,true),single_species(false)
{
  //  SetUpOrbits();
  Init(emax, e2max, e3max, reference, valence); // Init version 1  (int, string, string )
}
  ...
  sixj_has_been_precalculated(false),moshinsky_has_been_precalculated(false), scalar_transform_first_pass(true), scalar3b_transform_first_pass(true), tensor_transform_first_pass(40,true),single_species(false)
{
  ...
  if ( itval != ValenceSpaces.end() ) // we've got a valence space
    Init(emax,e2max,e3max,itval->second[0],valence); // Init version 1  ( int, string, string )
  else  // no valence space. we've got a single-reference.
    Init(emax,e2max,e3max,valence,valence); // Init version 1  ( int, string, string )
}
...
void ModelSpace::Init(int emax, int e2max, int e3max, std::string reference, std::string valence)
{
  //  std::cout << __func__ << "  line " << __LINE__ << std::endl;
  //  int Aref,Zref;
  ...
  //  }
  Init(emax, e2max, e3max, hole_list,valence);  // calls  Init version 2 (int, map, string)
}
...
void ModelSpace::Init(int emax, int e2max, int e3max, std::map<std::array<int,4>,double> hole_list, std::string valence)
{
  //  std::cout << __func__ << "  line " << __LINE__ << std::endl;
  //  int Ac,Zc;
  double Ac,Zc;
...
void ModelSpace::Init(int emax, int e2max, int e3max, std::vector<std::string> hole_list, std::vector<std::string> core_list, std::vector<std::string> valence_list)
{
  std::cout << __func__ << "  line " << __LINE__ << std::endl;
...
void ModelSpace::Init_occ_from_file(int emax, int e2max, int e3max, std::string valence, std::string occ_file)
{
  std::cout << __func__ << "  line " << __LINE__ << std::endl;
  //  index_t orb;
  double occ;
  ...
  Init(emax,e2max,e3max,hole_list,valence); // call Init version  2 ( int, map, string )
...
void ModelSpace::InitSingleSpecies(int emax, int e2max, int e3max, std::string reference, std::string valence)
{
  single_species = true;
  ClearVectors();
  GetAZfromString(reference,Aref,Zref);
  ...
  Init(emax,e2max,e3max,hole_list,valence);
...
void ModelSpace::SetupKets()
{
  ...
  for (auto p : all_orbits )
  {
    for (auto q : all_orbits )
    {
      ...
      Orbit & op = GetOrbit(p);
      Orbit & oq = GetOrbit(q);
      if( 2*op.n+op.l+2*oq.n+oq.l > E2max) continue;
      index_t index = Index2(p,q);
      //Kets[index] = Ket(GetOrbit(p),GetOrbit(q));
      Kets.push_back( Ket(GetOrbit(p),GetOrbit(q)));
      KetIndexLookup[index] = Kets.size()-1;
    }
  }
  for (auto p : all_orbits)
  {
    for (auto q : all_orbits)
    {
      ...
      Orbit & oq = GetOrbit(q);
      if( 2*op.n+op.l+2*oq.n+oq.l > E2max) continue;
      index_t index = KetIndexLookup.at(Index2(p,q));
      Ket& ket = Kets[index];
...
bool ModelSpace::CheckE2max(int a, int b)
{
  Orbit & oa = GetOrbit(a);
  Orbit & ob = GetOrbit(b);
  if( 2*(oa.n+ob.n)+oa.l+ob.l > E2max ) return false;
  return true;
}
