...
void TwoBodyChannel::Initialize(int j, int p, int t, ModelSpace *ms)
{
  J = j;
  parity = p;
  Tz = t;
...
ModelSpace::ModelSpace()
  :  Emax(0), E2max(0), E3max(0), Lmax2(0), Lmax3(0), norbits(0),
...
ModelSpace::ModelSpace(const ModelSpace& ms)
  :
    ...
    TwoBodyJPT2idx(ms.TwoBodyJPT2idx),
    ...
    nljtz2idx(ms.nljtz2idx), pq2idx(ms.pq2idx),
...
ModelSpace::ModelSpace(ModelSpace&& ms)
  :
    ...
    TwoBodyJPT2idx(std::move(ms.TwoBodyJPT2idx)),
...
ModelSpace::ModelSpace(int emax, int e2max, std::vector<std::string> hole_list, std::vector<std::string> valence_list)
  :  Emax(emax), E2max(e2max), E3max(3*emax), Lmax2(emax), Lmax3(emax), norbits(0), hbar_omega(20), target_mass(16),
...
ModelSpace::ModelSpace(int emax, int e2max, std::vector<std::string> hole_list, std::vector<std::string> core_list, std::vector<std::string> valence_list)
  : Emax(emax), E2max(e2max), E3max(3*emax), Lmax2(emax), Lmax3(emax), norbits(0), hbar_omega(20), target_mass(16),
...
ModelSpace::ModelSpace(int emax, int e2max, std::string reference, std::string valence)
  : Emax(emax), E2max(e2max), E3max(3*emax), Lmax2(emax), Lmax3(emax), hbar_omega(20),
  sixj_has_been_precalculated(false),moshinsky_has_been_precalculated(false), scalar_transform_first_pass(true), tensor_transform_first_pass(40,true)
{
  SetIndex1(emax);
...
ModelSpace::ModelSpace(int emax, int e2max, std::string valence)
  : Emax(emax), E2max(e2max), E3max(3*emax), Lmax2(emax), Lmax3(emax), hbar_omega(20),
  sixj_has_been_precalculated(false),moshinsky_has_been_precalculated(false), scalar_transform_first_pass(true), tensor_transform_first_pass(40,true)
{
  SetIndex1(emax);
...
void ModelSpace::SetIndex1(int emax)
{
  int indx = 0;
  for (int N=0; N<=Emax; ++N)
  {
    for (int l=N; l>=0; l-=2)
    {
      ...
      for (int j2=2*l+1; j2>=2*l-1 and j2>0; j2-=2)
      {
        for (int tz : {-1, 1} )
        {
          nljtz2idx[{n,l,j2,tz}] = indx;
          //std::cout << "indx: " << indx << ", n: " << n << ", l: " << l << ", j: " << j2 << ", tz: " << tz << std::endl;
          indx += 1;
        }
      }
    }
...
ModelSpace ModelSpace::operator=(ModelSpace&& ms)
{
  ...
  nljtz2idx = std::move(ms.nljtz2idx);
  pq2idx = std::move(ms.pq2idx);
